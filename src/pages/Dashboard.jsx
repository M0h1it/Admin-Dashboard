      ...
      <div className="w-64 min-h-screen bg-white-900 text-black flex flex-col p-4 shadow-2xl">
        ...
        <nav className="flex-1">
          {[{ icon: <DashboardIcon />, label: "Dashboard" },
            ...
            <div key={index} 
                 className={`flex items-center space-x-3 p-2 cursor-pointer hover:bg-blue-200 rounded-lg ${selectedSection === item.label ? "bg-blue-300" : ""}`} 
        ...
        <div className="flex items-center space-x-3 p-2 cursor-pointer hover:bg-blue-200 rounded-lg mt-auto" onClick={handleLogout}>
          <LogoutIcon />
          <span>Log Out</span>
        </div>
      </div>
      ...
      <div className="ml-.5 flex-1 bg-white-900">
        ...
        <Drawer anchor="right" open={openDrawer} onClose={() => setOpenDrawer(false)}>
          <Box className="w-96 p-6">
            ...
            <Button variant="contained" color="primary" fullWidth sx={{ mt: 2 }} onClick={() => setOpenDrawer(false)}>Save</Button>
          </Box>
        </Drawer>
